...
- **اتأكد فعلياً**: إنشاء منتج، بيع POS (نقص مخزون + خزينة + حركات + **قيد محاسبي متوازن**)، ومصروف — كلهم اتزامنوا صح على Supabase.
- **الباقي (المراحل الجاية)**: ترقية POS لكاشير حقيقي (باركود + بحث فوري + مرتجع من الفاتورة)، متجر أونلاين (كتالوج + أوردرات + خصومات، دفع عند الاستلام/إنستاباي + شحن)، رفع صور المنتجات، صقل التصميم، تقسيم bundle، وتسهيل إضافة موظفين (تأكيد البريد).
